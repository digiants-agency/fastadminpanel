...
function initGmapsContacts() {
    if (document.querySelectorAll('#map').length != 0) {
        ...
        var map_right = new google.maps.Map(document.querySelector('#map'), {
            center: {lat: 55.8621552, lng: 37.5571009},
            zoom: 13.04,
            draggable: true,
            scrollwheel: false,
        ...
        new google.maps.Marker({
            position: {lat: 55.862563, lng: 37.5477267},
            map: map_right,
        });
    }
}
...
$('.addtocartproductpage').on('click',function(){
   console.log('add');
    ...
    if($('.mods input').length>0) {
        product['mod'] = $('.mods input:checked').val();
        product['modtitle'] = $('.mods input:checked').data('title');
    }

    Cart.add({
        id: product['id'],
        attributes: [parseInt(product['mod'])],
    }, product['qty'], {
        id: product['id'],
        mod: parseInt(product['mod']),
        price: product['price'],
        sale_price: product['sale_price'],
        image: product['img'],
        title: product['title'],
        slug: product['slug'],
        attributes: [product['modtitle']],
    })
    console.log(product);
   return false;
});
...
function rendercart() {
    ...
    let sum = 0
        ...
        for (let [key, value] of Cart.cart.entries()) {
            if(value.data.sale_price>0) sum += value.count * (value.data.sale_price); else
                sum += value.count * (value.data.price);

            products_str += `<div class="item">
                <a class="delete" onclick="removecartitem(${value.data.id},${value.data.mod})">
                ...
                <div class="info">
                    ...
                    <span>${value.data.attributes[0]}</span>
                </div>
                <div class="priceblock">
                    <div class="qtyblock">
                        <span class="minusqty" onclick="changeqty(${value.data.id},${value.data.mod},'-')">-</span>
                        <input id="qtyproduct" type="number" min="1" max="40" value="${value.count}" readonly="">
                        <span class="plusqty" onclick="changeqty(${value.data.id},${value.data.mod},'+')">+</span>
                    </div>
                    <div class="price">`;
                    if(value.data.sale_price!=0){
                        products_str += `
                        <div class="oldprice">${value.data.price} грн</div>
                        <div class="price">${value.data.sale_price} грн</div>`;
                    } else {
                        products_str += `
                        <div class="price">${value.data.price} грн</div>`;
                    };
                        products_str +=`
                    </div>
                </div>
    ...
    $('.modal_cart .items').html(products_str);
        if($('.ordercart').length>0){
            ...
            $('.ordercart .finaltotal p').text(sum+' грн');
        }

    $('.modal_cart .totalprice').text(sum+' грн');
}

function changeqty(id,mod,typechange){
    if(typechange=='-'){
        Cart.changeqty({
            id: id,
            attributes: [mod],
...
function removecartitem(id, mod) {
    Cart.remove({
        id: id,
        attributes: [mod],
    });
...
$('#horform').submit(function(){
    var orderdata = $('#horform').serializeArray();
    $.ajax({
        type: 'POST',
        url: '/cart/callback',
        data: {orderdata: JSON.stringify(orderdata)},
        ...
        success: function (response) {
            if (response) {
                if (response != 'error') {
                    console.log('success');
                    $('#horform input.btn').val('Сообщение отправлено!');
                    return false;
                }
            }
            console.log('error');
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response) {
                if (response != 'error') {
                    console.log('success');
                    $('#callbackcontact input[type=submit]').val('Сообщение отправлено!');
                    return false;
                }
            }
            console.log('error');
...
$('#buyoneclick').submit(function(){
    var orderdata = $('#buyoneclick').serializeArray();
    $.ajax({
        type: 'POST',
...
$('.searchform input').on('input',function(){
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $('.ajaxsearchresult').html('Ничего не найдено');
            console.log('error');
...
$('#addtestim').submit(function () {
    var testimdata = $('#addtestim').serializeArray();
    $.ajax({
        type: 'POST',
        url: '/product/addtestim',
        data: testimdata,
        headers: {
            "Accept": "application/json",
            "X-CSRF-Token": document.querySelector('meta[name="csrf-token"]').getAttribute('content')
